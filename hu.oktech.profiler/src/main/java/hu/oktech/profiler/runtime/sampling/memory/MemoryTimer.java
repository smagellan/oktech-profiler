...
/**
 * Timer for memory allocation sampling
 * 
 * @author Istvan Soos
 */
public class MemoryTimer extends AbstractTimer {
	...
	protected MemorySampler sampler;

	public MemoryTimer(String config) {
		super();
		init(config);
	}
	...
	public void init(String config) {
		...
		this.samplingTimeMillis = Integer.parseInt(props.getProperty("millis", "10000"));
		...
		if ("local".equals(samplerName)) {
			sampler = new LocalMemorySampler();
		} else if (samplerName.startsWith("jmx/") || samplerName.startsWith("jmx-")) {
			sampler = new JMXMemorySampler(JmxConnectionUtils.get().get(samplerName.substring(4)));
		} else {
	...
	public void doTimedTask() {
		queue.add(sampler.getMemoryData());
	}
