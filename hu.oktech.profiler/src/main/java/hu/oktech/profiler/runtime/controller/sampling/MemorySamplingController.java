...
/**
 * Memory sampling controller
 * 
 * @author Istvan Soos
 * 
 */
public class MemorySamplingController extends AbstractTimerController {
    ...
    public void init(String config, Instrumentation instrumentation) {
        ...
        long millis = Long.parseLong(props.getProperty("millis", "10000"));
        String queue = props.getProperty("queue", "default");

        String prefix = props.getProperty("prefix", "basic-");

        String memoryTimerName = prefix + "memory";
        String dumpTimerName = prefix + "dump";

        TimerService timerService = TimerUtils.get();

        if (!timerService.exists(memoryTimerName))
            timerService.register(memoryTimerName, "sampling-memory", "queue=" + queue + ";millis="
                    + millis);
        ...
        this.timers = new String[] { memoryTimerName, dumpTimerName };
    }
