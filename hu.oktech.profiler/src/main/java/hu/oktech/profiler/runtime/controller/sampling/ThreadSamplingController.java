...
/**
 * Thread sampling controller
 * 
 * @author Istvan Soos
 * 
 */
public class ThreadSamplingController extends AbstractTimerController {
    ...
    public void init(String config, Instrumentation instrumentation) {
        ...
        long millis = Long.parseLong(props.getProperty("millis", "5000"));
        String queue = props.getProperty("queue", "default");

        String prefix = props.getProperty("prefix", "basic-");

        String threadTimerName = prefix + "thread";
        String dumpTimerName = prefix + "dump";

        TimerService timerService = TimerUtils.get();

        if (!timerService.exists(threadTimerName))
            timerService.register(threadTimerName, "sampling-thread", "queue=" + queue + ";millis="
                    + millis);
        ...
        this.timers = new String[] { threadTimerName, dumpTimerName };
    }
